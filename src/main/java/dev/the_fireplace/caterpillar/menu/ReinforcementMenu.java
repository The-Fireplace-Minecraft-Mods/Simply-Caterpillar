...
public class ReinforcementMenu extends AbstractScrollableMenu {
    ...
    protected void addSlots(Container inventory) {
        ...
        for (int column = 0; column <= 4; column++) {
            if (scrollToReplacer == REPLACER_CEILING) {
                super.addSlot(new Slot(inventory, slotId, REINFORCEMENT_SLOT_X_START + column * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START));
            }
            slotId++;
        }
        ...
        for (int row = 0; row <= 2; row++) {
            if (scrollToReplacer == REPLACER_LEFT) {
                super.addSlot(new Slot(inventory, slotId, REINFORCEMENT_SLOT_X_START, REINFORCEMENT_SLOT_Y_START + (1 + row) * SLOT_SIZE_PLUS_2));
            }
            slotId++;
        }
        ...
        for (int row = 0; row <= 2; row++) {
            if (scrollToReplacer == REPLACER_RIGHT) {
                super.addSlot(new Slot(inventory, slotId, REINFORCEMENT_SLOT_X_START + 4 * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START + (1 + row) * SLOT_SIZE_PLUS_2));
            }
            slotId++;
        }
        ...
        for (int column = 0; column <= 4; column++) {
            if (scrollToReplacer == REPLACER_FLOOR) {
                super.addSlot(new Slot(inventory, slotId, REINFORCEMENT_SLOT_X_START + column * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START + 4 * SLOT_SIZE_PLUS_2));
            }
            slotId++;
        }
    }
    ...
    public void scrollTo(int scrollToReplacer) {
        if (this.blockEntity instanceof ReinforcementBlockEntity reinforcementBlockEntity) {
            ...
            for (int column = 0; column <= 4; column++) {
                if (scrollToReplacer == REPLACER_CEILING) {
                    super.addSlot(new Slot(reinforcementBlockEntity, slotId, REINFORCEMENT_SLOT_X_START + column * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START));
                }
                slotId++;
            }
            ...
            for (int row = 0; row <= 2; row++) {
                if (scrollToReplacer == REPLACER_LEFT) {
                    super.addSlot(new Slot(reinforcementBlockEntity, slotId, REINFORCEMENT_SLOT_X_START, REINFORCEMENT_SLOT_Y_START + (1 + row) * SLOT_SIZE_PLUS_2));
                }
                slotId++;
            }
            ...
            for (int row = 0; row <= 2; row++) {
                if (scrollToReplacer == REPLACER_RIGHT) {
                    super.addSlot(new Slot(reinforcementBlockEntity, slotId, REINFORCEMENT_SLOT_X_START + 4 * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START + (1 + row) * SLOT_SIZE_PLUS_2));
                }
                slotId++;
            }
            ...
            for (int column = 0; column <= 4; column++) {
                if (scrollToReplacer == REPLACER_FLOOR) {
                    super.addSlot(new Slot(reinforcementBlockEntity, slotId, REINFORCEMENT_SLOT_X_START + column * SLOT_SIZE_PLUS_2, REINFORCEMENT_SLOT_Y_START + 4 * SLOT_SIZE_PLUS_2));
                }
                slotId++;
            }
        }
